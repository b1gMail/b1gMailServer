...
#ifndef WIN32
#ifndef __APPLE__
static bool logrotatePathOk(const char *path)
{
    if(path == NULL)
        return(false);
    size_t len = strlen(path);
    if(len < 2 || len > 200 || path[0] != '/')
        return(false);
    if(path[len - 1] == '/')
        return(false);
    if(strstr(path, "..") != NULL)
...
static bool logrotateFlagOn(const char *val, bool defaultOn)
{
    if(val == NULL || *val == '\0')
        return(defaultOn);
    if(strcmp(val, "0") == 0
...
static const char *logrotateInterval(const char *val)
{
    if(val == NULL)
        return("weekly");
    if(strcasecmp(val, "daily") == 0)
        return("daily");
    if(strcasecmp(val, "monthly") == 0)
        return("monthly");
    return("weekly");
}

static int logrotateCount(const char *val)
{
    int n = (val == NULL || *val == '\0') ? 8 : atoi(val);
    if(n < 1)
        n = 8;
    if(n > 99)
        n = 99;
...
static string logrotateBody(const char *logfile, const char *interval, int rotate, bool compress)
{
    char rotateBuf[16];
    snprintf(rotateBuf, sizeof(rotateBuf), "%d", rotate);

    string body;
    body += "# Generated by b1gMailServer from logfile= in b1gmailserver.cfg\n";
    body += logfile;
    body += " {\n    ";
    body += interval;
    body += "\n    rotate ";
...
void Utils::EnsureLogrotate()
{
    ...
    const char *logfile = cfg->Get("logfile");
    bool enabled = logrotateFlagOn(cfg->Get("logrotate"), false);

    if(!enabled || logfile == NULL || strlen(logfile) <= 2 || !logrotatePathOk(logfile))
    {
    ...
    string body = logrotateBody(
        logfile,
        logrotateInterval(cfg->Get("logrotate_interval")),
        logrotateCount(cfg->Get("logrotate_rotate")),
        logrotateFlagOn(cfg->Get("logrotate_compress"), true));
    writeLogrotateFile(optPath, body);
    if(FileExists("/etc/logrotate.d") && logrotateFileIsOurs(etcPath))
